...
1. Read the repo's guidance (`CLAUDE.md`/`AGENTS.md`/`README.md`) for the documented run/build/deploy
   procedure.
2. Detect the real chain from config: the manifest scripts (`dev`, `build`, `migrate`, `seed`), container
   setup (`docker`/compose files), env files (`.env`, `.env.example`), and any deploy config
   (`vercel.json`, CI workflows, Dockerfiles).
...
Report concisely: what you brought up/ran (with the exact commands), the health-check result (the endpoint
